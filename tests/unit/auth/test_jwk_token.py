...
def make_signing_server(mocker, key_set):
...
def mocked_signing_keys_server(mocker, single_key_set):
    return make_signing_server(mocker, single_key_set)
...
def token_header_384(multi_key_set):
    """A sample token header."""
    return {"alg": "RS384", "typ": "JWT", "kid": multi_key_set[1]["kid"]}
...
@pytest.fixture
def valid_tokens(multi_key_set, token_header, token_payload, token_header_384):
    """Generate valid tokens for each key in the multi-key set."""
    key_for_256 = multi_key_set[0]
    key_for_384 = multi_key_set[1]

    jwt_instance1 = JsonWebToken(algorithms=["RS256"])
    token1 = jwt_instance1.encode(
        token_header, token_payload, key_for_256["private_key"]
    ).decode()

    jwt_instance2 = JsonWebToken(algorithms=["RS384"])
...
def valid_tokens_no_kid(
    ...
    jwt_instance1 = JsonWebToken(algorithms=["RS256"])
    token1 = jwt_instance1.encode(
        token_header_256_no_kid, token_payload, key_for_256["private_key"]
    ).decode()
...
def multi_key_signing_server(mocker, multi_key_set):
    return make_signing_server(mocker, multi_key_set)
...
async def test_multi_key_valid(
    ...
    token1, token2 = valid_tokens
    set_auth_header(dummy_request, token1)

    dependency = JwkTokenAuthDependency(default_jwk_configuration)
    auth_tuple = await dependency(dummy_request)

    # Assert the expected values for the first token
    ensure_test_user_id_and_name(auth_tuple, token1)

    # Now test with the second token
    set_auth_header(dummy_request, token2)
    auth_tuple = await dependency(dummy_request)

    # Assert the expected values for the second token
...
async def test_multi_key_no_kid(
    ...
    token1, token2 = valid_tokens_no_kid
    ...
    # Should fail if no kid is specified and multiple keys are present and it's not the first key,
