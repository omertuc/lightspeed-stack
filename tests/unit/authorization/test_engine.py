...
from authorization.engine import JwtRolesResolver, GenericAccessResolver
...
class TestJwtRolesResolver:
    ...
    async def test_resolve_roles_redhat_employee(self):
        ...
        roles = await engine.resolve_roles(auth)
...
class TestGenericAccessResolver:
    ...
    async def test_check_access_with_valid_role(self):
        ...
        # Test access granted
        has_access = await resolver.check_access(Action.QUERY, frozenset(["employee"]))
        assert has_access is True

        # Test access denied
        has_access = await resolver.check_access(
            Action.FEEDBACK, frozenset(["employee"])
        )
    ...
    async def test_check_access_with_invalid_role(self):
        ...
        has_access = await resolver.check_access(Action.QUERY, frozenset(["visitor"]))
        assert has_access is False
    ...
    async def test_check_access_with_no_roles(self):
        ...
        has_access = await resolver.check_access(Action.QUERY, frozenset())
        assert has_access is False
